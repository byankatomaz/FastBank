...
def create_auth_JWT(email, password):
    ...
    print('autentificado com sucesso')
...
def avalicred_creation(headers, conta):
    ...
    print('avaliacao de credito feita')
    return response.json()
...
def main():
        ...
        user2 = cliente_creation('Julia', 'silvaJulinhaa@gmail.com', '02JU2904', '58263259845', 'PF', '13000', 'Rua Jose Afonso', 'Bairro das nascentes', 'Sumaré', 'São Paulo', 155, 13125642)
        headers_2 = create_auth_JWT('silvaJulinhaa@gmail.com', '02JU2904')

        user3 = cliente_creation('Beck', 'beckLuis@gmail.com', 'lUis123#$', '25632145685', 'PF', '15000', 'Rua Dos Papagaios', 'Arara Azul', 'Campinas', 'São Paulo', 132, 13625495)
        ...
        moviment12 = movimentacao_creation(headers_1, conta1.id, 100, 'TED', conta4.id)
        
        moviment2 = movimentacao_creation(headers_2, conta2.id, 45000, 'DEP')
        avaliacred2 = avalicred_creation(headers_2, conta2.id)
        emprestimo2 = emprestimo_creation(headers_2, conta2.id, 500, 12)
        moviment14 = movimentacao_creation(headers_1, conta2.id, 75, 'TED', conta3.id)
                  
        moviment3 = movimentacao_creation(headers_3, conta3.id, 25000, 'DEP')
        avaliacred3 = avalicred_creation(headers_3, conta3.id)
        emprestimo3 = emprestimo_creation(headers_3, conta3.id, 700, 12)
        moviment13 = movimentacao_creation(headers_1, conta3.id, 50, 'TED', conta2.id)

        
        moviment4 = movimentacao_creation(headers_4, conta4.id, 12500, 'DEP')
        avaliacred4 = avalicred_creation(headers_4, conta4.id)
        emprestimo4 = emprestimo_creation(headers_4, conta1.id, 250, 12)
        moviment14 = movimentacao_creation(headers_1, conta4.id, 25, 'TED', conta1.id)
